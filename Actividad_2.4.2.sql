...
FROM ATENCION A 
JOIN (SELECT PAC_RUN,DV_RUN,
        INITCAP(P.PNOMBRE||' '||P.SNOMBRE||' '||P.APATERNO||' '||P.AMATERNO) AS NOMBRE_PACIENTE,
...
GROUP BY A.PAC_RUN,P.DV_RUN,P.NOMBRE_PACIENTE,P.EDAD,PO.PORCENTAJE_DESCTO
HAVING COUNT(A.PAC_RUN) > 4;

--CASO 2
SELECT E.NOMBRE,
M.MED_RUN||'-'||M.DV_RUN AS "RUT MEDICO",
M.PNOMBRE||' '||M.SNOMBRE||' '||M.APATERNO AS "NOMBRE MEDICO"
FROM MEDICO M
JOIN ESPECIALIDAD_MEDICO EM ON EM.MED_RUN = M.MED_RUN
JOIN ESPECIALIDAD E ON E.ESP_ID = EM.ESP_ID
WHERE EM.ESP_ID NOT IN (SELECT E.ESP_ID
                    FROM ATENCION A
                    RIGHT JOIN ESPECIALIDAD E ON E.ESP_ID = A.ESP_ID
